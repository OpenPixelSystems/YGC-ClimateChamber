...
def api_clear_logs():
    ...
    try:
        ...
        if not log_dir.exists():
            return jsonify(
                {"success": True, "message": "Logs directory does not exist"}
            )
        ...
        return jsonify(
            {
                "success": True,
                "message": "All log files and folders cleared successfully",
            }
        )
    except Exception as e:
        return (
            jsonify({"success": False, "error": f"Error clearing logs: {str(e)}"}),
            500,
        )
