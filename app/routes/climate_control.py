...
from flask import Blueprint, jsonify, Response, request, render_template, flash, redirect, url_for
...
def start_sensors(logging=True):
    ...
    if app_state.database.logging_active:
        return jsonify({"status": "error", "message": "A cycle is already running"}), 400
...
def get_active_cycle_data():
    ...
    with app_state.database.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT name, start_time, origin_page FROM cycles WHERE cycle_id = ?", (app_state.database.current_cycle_id,))
        cycle_row = cursor.fetchone()
        cycle_name = cycle_row[0] if cycle_row else None
        cycle_start_time = cycle_row[1] if cycle_row else None
        origin_page = cycle_row[2] if cycle_row and len(cycle_row) > 2 else 'display-graph'
