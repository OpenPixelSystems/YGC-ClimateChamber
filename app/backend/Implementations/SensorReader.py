...
class SensorReader(ISensorReader, Subscriptable, LoggingMixin):
            ...
            for sensor_config in mcu_config.sensors:
                ...
                if sensor_config.type == "DS18B20":
                    ...
                    for sensor in self.sensor_list:
                        if (
                            isinstance(sensor, DS18B20Cluster)
                            and sensor.sensor_location == sensor_config.sensor_location
                        ):
                    ...
                    self.sensor_list.append(
                        MPL3115A2(
                            ...
                            sensor_config.SDA,
                            sensor_config.SCL,
                            sensor_config.min_value,
                            sensor_config.max_value,
                            sensor_config.unit,
                        )
                    )
    ...
    def read_inside_sensors(self):
        ...
        for sensor in self.inside_sensors_list:
            try:
                reading = (
                    sensor.read()
                )  # e.g., {'sensor_name': {'sensor_value': 19.9, 'sensor_source': 'real'}}
                ...
                self.print_error(
                    f"Error reading sensor {getattr(sensor, 'name', 'unknown')}: {e}"
                )
    ...
    def start_background_reading(self):
        ...
        self._background_thread = threading.Thread(
            target=self._background_read_loop, daemon=True
        )
        self._background_thread.start()
        self.print(
            f"[SensorReader] Background reading started (interval: {self._reading_interval}s)"
        )
    ...
    def _is_current_sensor(self, sensor_name: str) -> bool:
        ...
        return (
            "current" in sensor_name_lower
            or "amp" in sensor_name_lower
            or sensor_name_lower.endswith("_a")
        )
            ...
            if not self._peltier_enabled:
                ...
                for sensor_name, sensor_data in sensor_readings.items():
                    ...
                    if (
                        ...
                        and "sensor_value" in sensor_data
                        and "sensor_source" in sensor_data
                        and self._is_current_sensor(sensor_name)
                    ):
    ...
    def get_starting_temperature(self):
        ...
        try:
            ...
            for sensor_name, sensor_info in sensor_data.items():
                ...
                if (
                    not isinstance(sensor_info, dict)
                    or "sensor_value" not in sensor_info
                ):
                    continue
                ...
                for sensor in self.sensor_list:
                    ...
                    if sensor_name in sensor_names:
                        ...
                        if (
                            hasattr(sensor, "sensor_location")
                            and sensor.sensor_location == "Inside"
                        ):
                            # Check if it's NTC or DS18B20
                            if hasattr(sensor, "type") and sensor.type in [
                                "NTC",
                                "DS18B20",
                            ]:
                            ...
                            for sub_sensor in sensor.sensors:
                                if (
                                    sub_sensor.name == sensor_name
                                    and sub_sensor.sensor_location == "Inside"
                                ):
                                    temperature_values.append(sensor_value)
                                    break
                            break
            ...
            if temperature_values:
                ...
                self.print_error(
                    "[SensorReader] No viable inside temperature sensors found"
                )
                return None
