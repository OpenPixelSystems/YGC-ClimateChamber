...
class CalculationService(ICalculationService, Subscriptable, LoggingMixin):
    ...
    def __init__(
        self, config_manager: IConfigManager, use_adaptive_control: bool = False
    ):
        ...
        self.last_time = None
        self.last_pv = (
            None  # Last process variable (temperature) for derivative calculation
        )
        ...
        self.integral_limits = (
            -200.0,
            200.0,
        )  # Integral windup limits (allows Ki*200 max contribution)
    ...
    def _calculate_hvac_pid_control(
        self, current_temp, target_temp, current_time_offset=None
    ):
        ...
        if self.setpoint_schedule and current_time_offset is not None:
            feedforward = self._calculate_conservative_feedforward(
                filtered_temp, filtered_setpoint, current_time_offset
            )
            raw_output += feedforward

        # Apply output limits
        limited_output = max(
            self.output_limits[0], min(self.output_limits[1], raw_output)
        )
    ...
    def _is_output_saturated(self):
        ...
        return (
            self.current_output <= self.output_limits[0]
            or self.current_output >= self.output_limits[1]
        )
    ...
    def _apply_minimum_times(self, desired_output, dt):
        ...
        if self.last_significant_output_time:
            time_since_change = (
                current_time - self.last_significant_output_time
            ).total_seconds()
        ...
        if (
            (was_heating and not is_heating)
            or (was_cooling and not is_cooling)
            or (was_off and not is_off)
        ):
    ...
    def _get_future_setpoints(self, current_time_offset, horizon_seconds=60):
        ...
        if not self.setpoint_schedule or not hasattr(
            self.setpoint_schedule, "interpolated_setpoints"
        ):
            return []
        ...
        for time_offset, temp in self.setpoint_schedule.interpolated_setpoints:
            if (
                current_time_offset
                < time_offset
                <= current_time_offset + horizon_seconds
            ):
                future_setpoints.append((time_offset, temp))

        return future_setpoints

    def _calculate_conservative_feedforward(
        self, current_temp, current_target, current_time_offset
    ):
        """Conservative feedforward for HVAC applications."""
        if not self.setpoint_schedule:
            return 0

        future_setpoints = self._get_future_setpoints(
            current_time_offset, self.prediction_horizon
        )
        ...
        for time_offset, temp in future_setpoints:
            ...
            if abs(temp_change) > 2.0:  # Only act on significant changes
                ...
                if time_to_change > 0:
                    ...
                    ff_gain = self.feedforward_gain * (
                        1.0 - time_to_change / self.prediction_horizon
                    )
    ...
    def _log_pid_data(self, temp, setpoint, error, output, status, details=None):
        ...
        if status == "ACTIVE":
            self.print(
                f"[HVAC_PID] {mode} | Temp: {temp:.1f}°C -> Target: {setpoint:.1f}°C | Output: {output:.1f}%"
            )
