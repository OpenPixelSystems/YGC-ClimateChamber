...
class AppState(metaclass=SingletonMeta):
    ...
    def _create_database_manager(self):
        ...
        return DatabaseManager(
            "database.db", self.sensor_reader, self.calculation_service
        )
    ...
    def restart_climate_chamber_service(self):
        ...
        try:
            print(
                "[app_state] [restart_climate_chamber_service] Restarting climate chamber"
            )
            subprocess.run(
                ["sudo", "systemctl", "restart", "climatechamber.service"], check=True
            )
            # Exit the current process cleanly
            sys.exit(0)
        except subprocess.CalledProcessError as e:
            print(
                f"[app_state] [restart_climate_chamber_service] Failed to restart service: {e}"
            )
            sys.exit(1)
