...
class TemperatureService:
    ...
    def __init__(self, config_manager: IConfigManager, controller: ClimateChamberController):
        self.config_manager = config_manager
        self.controller = controller
    
    def validate_temperature(self, temp_value: Union[int, float, str]) -> TemperatureValidationResult:
    ...
    def set_constant_temperature(self, temperature: Union[int, float, str]) -> TemperatureValidationResult:
        ...
        try:
            self.controller.desired_flow_graph = Graph('desired_temperature', [(0, float(temperature))], self.config_manager)
            return TemperatureValidationResult(
                is_valid=True,
    ...
    def set_temperature_profile(self, points: List[Dict[str, Any]]) -> Tuple[bool, str, Optional[Graph]]:
