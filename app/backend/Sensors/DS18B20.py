...
class DS18B20(ISensor):
    ...
    def initialize(self) -> None:
        ...
        print(
            f"[DS18B20] [initialize] Initializing DS18B20 with code: {self._full_rom_address}"
        )

        if self._is_testing:
            print(
                f"[DS18B20] [initialize] Testing mode enabled - using simulated values"
            )
            return
        ...
        try:
            self._sensor = W1ThermSensor(
                sensor_type=Sensor.DS18B20, sensor_id=self._serial_code
            )
            ...
            print(
                f"[DS18B20] [initialize] SUCCESS: DS18B20 sensor '{self._full_rom_address}' connected"
            )
            print(f"[DS18B20] [initialize] Current temperature: {test_temp}°C")
            print(
                f"[DS18B20] [initialize] Note: Resolution should be set via /etc/rc.local for optimal performance"
            )

        except Exception as e:
            print(
                f"[DS18B20] [initialize] ERROR: Failed to initialize sensor '{self._full_rom_address}': {str(e)}"
            )
            self._sensor = None
            ...
            try:
                ...
                if available_sensors:
                    ...
                    print(
                        f"[DS18B20] [initialize] Please check if ROM address '{self._serial_code}' is correct"
                    )
                else:
                    print(f"[DS18B20] [initialize]   - No DS18B20 sensors found")
                    print(
                        f"[DS18B20] [initialize] Check sensor wiring and 1-Wire configuration"
                    )
            except Exception as list_error:
                print(
                    f"[DS18B20] [initialize] Could not list available sensors: {str(list_error)}"
                )
                print(
    ...
    def _get_simulated_value(self) -> float:
        ...
        try:
            ...
            print(
                f"[DS18B20] Warning: Temperature simulation failed, using fallback: {e}"
            )
    ...
    def _read_sensor_with_timeout(self) -> Optional[float]:
        ...
        if thread.is_alive():
            print(
                f"[DS18B20] Sensor {self._full_rom_address} read timed out after {self._read_timeout}s"
            )
            return None

        if exception_info[0]:
            print(
                f"[DS18B20] Sensor {self._full_rom_address} read failed: {exception_info[0]}"
            )
            return None
    ...
    def read(self) -> Dict[str, Dict[str, Optional[float]]]:
        ...
        try:
            if self._is_testing:
                ...
                print(
                    f"[DS18B20] Testing mode - simulated value for {self._full_rom_address}: {value} {self._unit}"
                )
            else:
                ...
                if temp_c is not None:
                    ...
                    self._consecutive_failures = 0
                    print(
                        f"[DS18B20] Read sensor {self._full_rom_address}: {rounded_temp} {self._unit}"
                    )
                else:
                    # Failed read - increment failure counter
                    self._consecutive_failures += 1
                    print(
                        f"[DS18B20] Failed to read sensor {self._full_rom_address} (failure #{self._consecutive_failures})"
                    )

                    # Use fallback strategy based on failure count
                    if (
                        self._consecutive_failures <= self._max_consecutive_failures
                        and self._last_successful_reading is not None
                    ):
                        ...
                        print(
                            f"[DS18B20] Using fallback value for {self._full_rom_address}: {fallback_value} {self._unit}"
                        )
                    else:
                        # Too many consecutive failures, return None
                        print(
                            f"[DS18B20] Max consecutive failures ({self._max_consecutive_failures}) reached for {self._full_rom_address}, returning None"
                        )
            ...
            print(
                f"[DS18B20] Critical error reading sensor {self._name} ({self._full_rom_address}): {str(e)}"
            )
            self._consecutive_failures += 1
