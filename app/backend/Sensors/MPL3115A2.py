    ...
    def initialize(self) -> None:
        ...
        if not self._is_testing:
            try:
                ...
                i2c = busio.I2C(
                    scl=getattr(board, f"D{self._scl_pin}", board.SCL),
                ...
                self._sensor.sealevel_pressure = (
                    1013.25  # Adjust based on local sea-level pressure
                )
                ...
                print(
                    f"[MPL3115A2] Failed to initialize real sensor: {str(e)}, switching to testing mode"
                )
    ...
    def _get_simulated_values(self) -> Dict[str, float]:
        ...
        try:
            ...
            new_temperature = simulation_service.get_simulated_temperature(
                sensor_name=f"{self._name}_temperature", min_temp=-40.0, max_temp=85.0
            )
        except Exception as e:
            print(
                f"[MPL3115A2] Warning: Temperature simulation failed, using fallback: {e}"
            )
        ...
        try:
            if not self._is_testing and self._sensor is not None:
                ...
                try:
                    ...
                    print(
                        f"[MPL3115A2] Read actual sensor {measurement_type}: {value:.2f} {self._unit}"
                    )
                    ...
                    print(
                        f"[MPL3115A2] Error reading real sensor: {str(e)}, returning None"
                    )
            else:
                ...
                if self._is_testing:
                    ...
                    print(
                        f"[MPL3115A2] In testing environment, using simulated value: {value} {self._unit}"
                    )
                else:
                    ...
                    print(
                        f"[MPL3115A2] Sensor initialization failed on real hardware, returning None"
                    )

        except Exception as e:
            print(f"[MPL3115A2] Critical error reading sensor {self._name}: {str(e)}")
            data_source_key = f"{self._name}_data_source"
            json_format[self._type] = {self._name: None, data_source_key: "error"}
        ...
        try:
            if not self._is_testing and self._sensor is not None:
                ...
                try:
                    ...
                    print(
                        f"[MPL3115A2] Error reading real sensor: {str(e)}, returning None values"
                    )
            else:
                # We're in a test environment or sensor initialization failed
                # Only use simulated values in testing environment
                if self._is_testing:
                    ...
                    print(
                        f"[MPL3115A2] Sensor initialization failed on real hardware, returning None values"
                    )
